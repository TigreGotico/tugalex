...
class TugaLexicon:
    ...
    def __init__(self, dictionary_path: Optional[str] = None) -> None:
        ...
        Args:
            dictionary_path (Optional[str]): Path to the CSV file (optionally
                gzip-compressed) containing regional phoneme and syllable
                mappings. If None, defaults to "regional_dict.csv.gz" in the
                data directory — a slim (word, pos, phones, syllables,
                region_code) extract of the unified Portuguese pronunciation
                lexicon (TigreGotico/portuguese-unified-pronunciation-lexicon),
                regenerated by scripts/build_regional_dict.py.
        """
        self.dictionary_path: str = dictionary_path or os.path.join(
            os.path.dirname(__file__), "data", "regional_dict.csv.gz"
        )
    ...
    def _load_lang_map(path: str) -> Tuple[IPA_MAP, SYLLABLE_MAP, Set[str]]:
        """
        Load phoneme and syllable mappings from a CSV into region-indexed
        lookup structures.
